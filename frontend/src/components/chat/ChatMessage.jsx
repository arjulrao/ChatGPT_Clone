...
const ChatMessages = ({ messages, isSending }) => {
  ...
  // Always scroll to bottom when new messages are added (user requested default latest view)
  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: 'smooth', block: 'end' });
    setIsAtBottom(true);
  }, [messages.length]);

  // Track scroll position to toggle buttons
  const onScroll = () => {
    ...
    setShowTopBtn(el.scrollTop > 400);
  };
  ...
  return (
    <div className="messages" ref={containerRef} onScroll={onScroll} aria-live="polite">
      ...
      {!isAtBottom && (
        <button type="button" className="scroll-jump scroll-jump-latest" aria-label="Scroll to latest message" onClick={scrollToBottom}>
          ↓ Latest
        </button>
      )}
      {showTopBtn && (
        <button type="button" className="scroll-jump scroll-jump-top" aria-label="Scroll to first message" onClick={scrollToTop}>
          ↑ Top
        </button>
      )}
    </div>
